...
def add_card_content(slide, left, top, width, height, heading, body_items, accent_color=None, badge_text="", heading_size=19, body_size=14, space_between=4):
    ...
    for idx, item in enumerate(body_items):
        p = tf.add_paragraph()
        p.space_before = Pt(space_between if idx > 0 else 6)
        p.line_spacing = 1.18
...
def create_full_presentation():
    ...
    add_bottom_callout(s12, "Quantified ROI: 98.6% time reduction with 100% verified footnote provenance across all filings.", highlight_label="Impact Summary:")

    # ═════════════════════════════════════════════════════════
    # SLIDE 13 — 12. KEY LEARNINGS & FUTURE ROADMAP
    # ═════════════════════════════════════════════════════════
    s13 = prs.slides.add_slide(blank_layout)
    set_slide_background(s13)
    add_slide_header(s13, "12. Key Learnings & Future Roadmap", "Technical competencies developed during the internship and commercial product roadmap.")
    
    col_w13 = Inches(5.8)
    col_g13 = Inches(0.233)
    h13     = Inches(4.35)
    top13   = Inches(1.78)
    
    add_card_content(
        s13, MARGIN_L, top13, col_w13, h13,
        "Engineering Competencies Gained",
        [
            ("Multi-Agent Orchestration:", "LangGraph state machine modeling, cyclic transitions, and deterministic state checkpoints in MongoDB."),
            ("Hybrid RAG Systems:", "Combining 384-dim dense vector search, BM25 lexical keyword matching, and cross-encoder re-ranking."),
            ("LLM Reliability & Fallbacks:", "Multi-provider resilience with Groq Cloud LLaMA 3.3 and NVIDIA Nemotron 550B circuit breakers."),
            ("Full-Stack Enterprise Engineering:", "FastAPI ASGI async endpoints, React 19 UI, and dynamic two-pass ReportLab PDF generation.")
        ],
        accent_color=ACCENT_TEAL, badge_text="LEARNINGS", heading_size=20, body_size=13.5, space_between=6
    )
    
    add_card_content(
        s13, MARGIN_L + col_w13 + col_g13, top13, col_w13, h13,
        "Future Product Roadmap",
        [
            ("Q3 — SEC EDGAR Real-Time Sync:", "Automated ingestion pipeline directly from SEC EDGAR RSS feeds for instant quarterly earnings processing."),
            ("Q4 — Audio Earnings Calls:", "Whisper transcription and multi-modal tone/sentiment analysis on quarterly analyst conference calls."),
            ("Q1 — Multi-Document Graph RAG:", "Cross-quarter Knowledge Graph linking corporate subsidiaries, key suppliers, and competitor entities."),
            ("Q2 — Enterprise SSO & On-Premise:", "SAML/OAuth2 integration with air-gapped private LLM deployment for financial institutions.")
        ],
        accent_color=ACCENT_CYAN, badge_text="ROADMAP", heading_size=20, body_size=13.5, space_between=6
    )
    
    add_bottom_callout(s13, "Thank You  ·  Velsora: Multi-Agent AI Analysis System  ·  Questions & Answers", highlight_label="Conclusion:")
